...
export async function GenerateUploaderURL(fileInfo: {
  ...
  const s3client = new S3Client({
    ...
    // Prevent the SDK from auto-injecting a CRC32 placeholder
    // (x-amz-checksum-crc32=AAAAAA==) into presigned PUT URLs.
    // Browsers cannot compute CRC32 at upload time, so the default
    // WHEN_SUPPORTED setting causes every browser-side upload to fail
    // with a checksum mismatch after the CORS preflight succeeds.
